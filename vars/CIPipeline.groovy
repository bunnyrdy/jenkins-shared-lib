...
def call(Map config) {
    pipeline {
        ...
        stages {
            stage('Read version') {
                steps {
                    echo "reading project: ${env.PROJECT}, component: ${env.COMPONENT}"
                }
            }

            stage('Unit test') {
                steps {
                    sh 'echo "Unit tests are created by developers — e.g. npm test"'
                }
            }
